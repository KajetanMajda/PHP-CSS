		...
		<nav class="navbar navbar-dark bg-primary justify-content-between">
			...
			<h1>Capto</h1>
  				...
  				<form class="form-inline" method="post">
    				<input class="form-control mr-sm-2" type="search" placeholder="Wyszukaj" aria-label="Wyszukaj" name="sercz">
	...
	<main>
		...
		<?php
			...
			if(isset($_POST['sercz']) && !empty($_POST['sercz'])){
				...
				FROM `tools` WHERE dzial=12 AND numer_katalogowy 
				LIKE "%'.$_POST['sercz'].'%"';
    	    	...
    	    	$sql = 'SELECT `nazwa`, `numer_narzedzia`, `numer_katalogowy`, `link` FROM `tools` WHERE dzial=12 ORDER BY `numer_narzedzia`';
    	    	$result = $baza->query($sql);
  					if ($result->num_rows > 0) {
    	  			while ($row = $result->fetch_assoc()) {
